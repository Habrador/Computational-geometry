...
namespace Habrador_Computational_Geometry
{
    ...
    {
        public static HalfEdgeData GenerateTriangulation(HashSet<Vector3> points, List<Vector3> constraints, bool shouldRemoveTriangles, HalfEdgeData triangleData)
        {
        ...
        private static HalfEdgeData AddConstraints(HalfEdgeData triangleData, List<Vector3> constraints, bool shouldRemoveTriangles)
        {
            ...
            for (int i = 0; i < constraints.Count; i++)
            {
                ...
                List<HalfEdge> intersectingEdges = FindIntersectingEdges(uniqueEdges, v_i, v_j);
        ...
        private static void RestoreDelaunayTriangulation(Vector3 v_i, Vector3 v_j, List<HalfEdge> newEdges)
        {
            ...
            while (true)
            {
                ...
                for (int j = 0; j < newEdges.Count; j++)
                {
                    ...
                    if (_Delaunay.ShouldFlipEdge(v_l.XZ(), v_k.XZ(), v_third_pos.XZ(), v_opposite_pos.XZ()))
                    {
                        //Flip the edge
                        hasFlippedEdge = true;
        ...
        private static void RemoveSuperfluousTriangles(HalfEdgeData triangleData, List<Vector3> constraints)
        {
        ...
        public static HashSet<HalfEdgeFace> FindTrianglesWithinConstraint(HalfEdgeData triangleData, List<Vector3> constraints)
        {
            if (constraints.Count < 3)
            {
                return null;
            ...
            HalfEdgeFace borderTriangle = null;

            Vector3 constrained_p1 = constraints[0];
            Vector3 constrained_p2 = constraints[1];

            foreach (HalfEdgeFace t in triangleData.faces)
            {
                HalfEdge e1 = t.edge;
            ...
            HashSet<HalfEdgeFace> trianglesWithinConstraint = new HashSet<HalfEdgeFace>();
        ...
        private static List<HalfEdge> FindIntersectingEdges(List<HalfEdge> uniqueEdges, Vector3 p1, Vector3 p2)
        {
        ...
        //This assumes there are no holes in the mesh
        private static void FindIntersectingEdgesWithTriangleWalk(HalfEdgeData triangleData, Vector3 p1, Vector3 p2, List<HalfEdge> intersectingEdges)
        {
            //Step 1. Begin at a triangle connected to the first vertex in the constraint edge
            HalfEdgeFace f = null;

            foreach (HalfEdgeFace testFace in triangleData.faces)
            {
                //The edges the triangle consists of
                HalfEdge e1 = testFace.edge;
                HalfEdge e2 = e1.nextEdge;
                HalfEdge e3 = e2.nextEdge;
        ...
        private static bool IsEdgePartOfTriangulation(List<HalfEdge> uniqueEdges, Vector3 p1, Vector3 p2)
        {
            ...
            for (int i = 0; i < uniqueEdges.Count; i++)
            {
                ...
                Vector3 e_p1 = uniqueEdges[i].v.position;
                Vector3 e_p2 = uniqueEdges[i].prevEdge.v.position;
                ...
                if ((e_p1 == p1 && e_p2 == p2) || (e_p1 == p2 && e_p2 == p1))
                {
                    return true;
                }
            }
